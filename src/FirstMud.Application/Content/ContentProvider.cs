...
public sealed class ContentProvider : IContentProvider
{
    ...
    public void Reload()
    {
        ...
        // Factions load after zones would load (see task note: "if load order
        // matters, put factions after zones"). Zones are not yet present on
        // this base branch — when they are, LoadFactions will cross-check
        // hqZoneId / waypointZoneIds against the zones registry.
        _factions = LoadFactions();
        _factionsById = _factions.ToDictionary(f => f.Id);
        _logger?.LogInformation(
            "ContentProvider loaded: {ConsumableCount} consumables, {BuildingCount} buildings, {RecipeCount} recipes, {MonsterCount} monsters, {PoolCount} drop pools, {MonsterDropCount} monster drops, {FactionCount} factions from {Root}",
            _consumables.Count, _buildings.Count, _recipes.Count, _monsters.Count, _lootTables.DropPools.Count, _lootTables.MonsterDrops.Count, _factions.Count, _contentRoot);
    }
